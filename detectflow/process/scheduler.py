...
class Scheduler(ConfigHandler):
    ...
    def submit_job(self, job_script):
        ...
        ssh_command = ['ssh', 'chlupp@zenith.metacentrum.cz', 'qsub', job_script]
    ...
    def fetch_job_info(self, job_id):
        ...
        try:
            ...
            ssh_command = ['ssh', 'chlupp@zenith.metacentrum.cz', 'qstat', '-f', job_id]
            result = subprocess.run(ssh_command, capture_output=True, text=True)
            if result.returncode != 0:
                # If job is not in the current list, try fetching finished job details
                ssh_command = ['ssh', 'chlupp@zenith.metacentrum.cz', 'qstat', '-xf', job_id]
                result = subprocess.run(ssh_command, capture_output=True, text=True)
                if result.returncode != 0:
